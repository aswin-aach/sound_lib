...
Array* read_file(char* file_path) {
	...
	Array* buffer = create_array(sample_count, sample_size);
...
Array* read_tracked_file(char* file_path, int track_count) {
	...
	size_t sample_size = sizeof(sample_type);
	Array* track_array = create_array(track_count, sizeof(Array));
...
static int is_invalid(Array* array) {
	if (array == NULL || array->elements == NULL || array->length < 1) {
...
Array* add(Array* source, Array* target, long offset) {
	...
	int* source_sample = (int*) source->elements;
	int* target_sample = (int*) target->elements;
